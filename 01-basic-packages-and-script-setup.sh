...
DEBIAN_FRONTEND=noninteractive apt -y install vim chrony openssh-server screen net-tools git mc postfix sendemail tmux \
    tar sudo wget curl ethtool iptraf-ng traceroute telnet rsyslog software-properties-common \
    dirmngr htop iotop sysstat parted gdisk apt-transport-https lsb-release ca-certificates iputils-ping \
    whiptail wireguard locales wget fdisk parted bridge-utils iptables jq conntrack gnupg nfs-common socat ipset \
    rsyslog-kubernetes pwgen cpu-checker kubetail kubecolor ebtables elinks iftop vnstat
